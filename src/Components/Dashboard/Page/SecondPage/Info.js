...
const Info = () => {
	...
	useEffect(() => {
		axios
			.get("../../db.json")
			.then((res) => {
				setItems(res.data.secondPage[count]);
				setTotal(res.data.secondPage.length);
			})
			...
			<div className="border-bg p-1 p-sm-2 info second__box">
				...
				<div className="p__abs d-flex justify-content-between align-items-center">
					<div className="d-flex flex-nowrap ab">
						...
						<span className="bg-logo rounded spn d-inline-block bg">{items.id}</span>
					</div>
									...
									<div className="first__row d-flex justify-content-between">
										<span className="d-inline-block border rounded-circle">
											{items.people[0].id === 1 && items.people[0].link ? (
												<Link className="rounded-circle d-inline-block w-100 h-100 bg-logo" to="/home/page2"></Link>
											) : (
												""
											)}
										</span>
								...
								<div className="sec__d">
									...
									<div className="secd__row d-flex justify-content-between">
										<div className="first">
											...
											<span className="d-inline-block border rounded-circle">
												{items.people[0].followers[1].id === 1 && items.people[0].followers[1].link ? (
													<Link
														className="rounded-circle d-inline-block w-100 h-100 bg-pry"
														to="/home/page2"
													></Link>
										...
										<div className="second">
											...
											<span className="d-inline-block border rounded-circle">
												{items.people[1].followers[1].id === 1 && items.people[1].followers[1].link ? (
													<Link
														className="rounded-circle d-inline-block w-100 h-100 bg-pry"
														to="/home/page2"
													></Link>
